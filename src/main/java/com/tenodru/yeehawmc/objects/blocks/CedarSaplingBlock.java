...
public class CedarSaplingBlock extends BushBlock implements IGrowable{
	
	public static final IntegerProperty STAGE = BlockStateProperties.STAGE_0_1;
	protected static final VoxelShape SHAPE = Block.makeCuboidShape(2.0D, 0.0D, 2.0D, 14.0D, 12.0D, 14.0D);
	private final Supplier<Tree> tree;

	public CedarSaplingBlock(Supplier<Tree> treeIn, Properties properties) {
		super(properties);
		this.tree = treeIn;
	}
	...
	public void tick(BlockState state, ServerWorld worldIn, BlockPos pos, Random rand) {
		...
		if (worldIn.getLight(pos.up()) >= 9 && rand.nextInt(7) == 7) {
	...
	// Does random math to determine whether bonemeal can be used or not
	@Override
